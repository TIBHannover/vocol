...
var data = shell.exec('find ins1/master -type f -name \'*.ttl\'', {
  silent: false
});
...
shell.cd('../vocol/helper/tools/apache-jena-fuseki/').stdout;
...
for (var i = 0; i < files.length - 1; i++) {
  ...
  var output = shell.exec(
    './bin/s-put  http://localhost:3030/dataset  ' + files[i] +
    '  ../../../../repoFolder/' +
    files[i], {
      silent: false
    });
  console.log('./bin/s-put  http://localhost:3030/dataset  ' + files[i] +
    '  ../../../../repoFolder/' +
    files[i]);
